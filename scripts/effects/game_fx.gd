...
static var spark_pool: Node = null

## Called once by main at startup with the world effect containers.
static func register(fx: Node2D, pool: Node2D, sparks: Node) -> void:
	fx_layer = fx
	damage_pool = pool
	spark_pool = sparks

## Spawns a one-shot death burst at a world position using the enemy's palette,
## plus a small spark overlay on top for extra pop (does NOT replace the burst).
...
## Plays a pooled spark burst at a world position (projectile impacts, deaths).
static func sparks_at(world_pos: Vector2) -> void:
	if spark_pool == null or not is_instance_valid(spark_pool):
		return
	if spark_pool.has_method("spawn"):
		spark_pool.spawn(world_pos)
